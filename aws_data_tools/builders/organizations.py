...
class OrganizationDataBuilder(ModelBase):
    ...
    def __e_policies(self) -> List[Dict[str, Any]]:
        ...
        for p in self.dm.available_policy_types:
            ...
            for p_summary in p_summaries:
                p_detail = self.api('describe_policy', policy_id=p_summary['id']).get('policy')
    ...
    def __t_policy_targets(self) -> Dict[str,
                                         Dict[str,
                                              List[Union[PolicySummaryForTarget,
                                                         PolicySummary]
                                                  ]
                                             ]
                                        ]:
    ...
    def __e_effective_policies_for_target(self, target_id: str) -> List[EffectivePolicy]:
        ...
        for p in self.dm.available_policy_types:
            ...
            if p.type == 'SERVICE_CONTROL_POLICY':
                continue
            data = self.api('describe_effective_policy', policy_type=p.type, target_id=target_id)
