...
# # - split each line into list of path elements (split on /)
# # - get unique elements from the top-level path
# # - create dict with each top-level path as a key

# Create a set of OUs:
# - root OU with no child OUs
# - root OU with one-level of children
# - root OU with two-levels of children
awslocal organizations create-ou --parent-id "$root_id" \
                                 --name "No Children" \
                                 --output text
